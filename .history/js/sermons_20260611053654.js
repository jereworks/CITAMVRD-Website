...
class SermonApp {
    constructor() {
        this.cacheKey = 'citam_sermons_v3';
    ...
    async loadData() {
        ...
        // Fetch videos from the uploads playlist (up to 3 pages / 150 videos)
        const videos = [];
        let pageToken = '';
            ...
            vidData.items.forEach(item => {
                if (item.contentDetails && item.snippet) {
                    videos.push({
                        ...
                        playlists: [],  // populated below from playlist membership map
        ...
        // For every playlist, fetch its video IDs in parallel (up to 2 pages / 100 items each).
        // This is the fix: previously all videos were tagged playlist:'All' and never matched
        // any filter. Now we build a real videoId → [playlistTitles] map.
